...
# C++20 でコンパイルして input.txt から入力、output.txt へ出力
runcppio() {
...
# WezTerm OSC 7 support to track CWD
if [[ "$TERM_PROGRAM" == "WezTerm" ]]; then
  prompt_osc7() {
    printf '\033]7;file://%s%s\033\\' "$HOSTNAME" "$PWD"
  }
  if [[ ! "$PROMPT_COMMAND" =~ prompt_osc7 ]]; then
    PROMPT_COMMAND="prompt_osc7;${PROMPT_COMMAND}"
  fi
fi
